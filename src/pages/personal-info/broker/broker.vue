...
import ListItem from "../lawyer/components/list-item.vue";
import { ref } from "vue";
...
const tabsList = ref<Array<any>>([
  { name: t("user.broker.current") },
  { name: t("user.broker.history") },
]);
...
const getBrokerList = async () => {
  list.value = await userStore.getBrokerList(current.value);
};

    ...
    <view class="oper-wrapper" :style="{ display: sD, top: tp }">
      <view class="oper">
        <view class="btn-wrapper" v-if="sO">
          <view @click="fSelect" hover-class="hover" :style="{ width: bW }"
            ><text>重选</text></view
          >
          <view @click="fClose" hover-class="hover" :style="{ width: bW }"
            ><text>关闭</text></view
          >
          <view
            @click="fRotate"
            hover-class="hover"
            :style="{ width: bW, display: bD }"
            ><text>旋转</text></view
          >
        ...
        <view class="clr-wrapper" v-else>
          <slider
            ...
            show-value
          ></slider>
          <view @click="fPrvUpload" hover-class="hover" :style="{ width: bW }"
            ><text>上传</text></view
          >
        </view>
      </view>
...
export default {
  ...
  created() {
    ...
    this.letRotate =
      ...
      this.canRotate === "false"
        ...
        : 1;
    this.letScale =
      this.canScale === false || this.canScale === "false" ? 0 : 1;
  ...
  methods: {
    fWindowResize() {
      ...
      // #ifdef MP-ALIPAY
      this.wH =
        sysInfo.screenHeight - sysInfo.statusBarHeight - sysInfo.titleBarHeight;
      ...
      this.expWidth &&
        (this.eW =
          this.expWidth.toString().indexOf("upx") >= 0
            ? parseInt(this.expWidth) * this.pxRatio
            : parseInt(this.expWidth));
      this.expHeight &&
        (this.eH =
          this.expHeight.toString().indexOf("upx") >= 0
            ...
            success: (r) => {
              ...
              if (!this.hasSel) {
                ...
                if (this.selWidth && this.selHeight) {
                  let sW =
                      this.selWidth.toString().indexOf("upx") >= 0
                        ? parseInt(this.selWidth) * this.pxRatio
                        : parseInt(this.selWidth),
                    sH =
                      this.selHeight.toString().indexOf("upx") >= 0
    ...
    fDrawImage(ini = false) {
      ...
      if (this.isin) {
        ...
        cc.drawImage(
          this.imgPath,
          this.posWidth - this.centerX - cx,
          this.posHeight - this.centerY - cy,
          imgWidth,
          imgHeight
        );
      } else {
        cc.translate(
          this.posWidth + imgWidth / 2,
          this.posHeight + imgHeight / 2
        );
        cc.rotate((this.rotateDeg * Math.PI) / 180);
        cc.drawImage(
          this.imgPath,
    ...
    fChooseImg(index = undefined, params = undefined, data = undefined) {
      if (params) {
        ...
        expWidth &&
          (this.eW =
            expWidth.toString().indexOf("upx") >= 0
              ? parseInt(expWidth) * this.pxRatio
              : parseInt(expWidth));
        expHeight &&
          (this.eH =
            expHeight.toString().indexOf("upx") >= 0
        ...
        if (sW && sH) {
          sW =
            sW.toString().indexOf("upx") >= 0
              ? parseInt(sW) * this.pxRatio
              : parseInt(sW);
          sH =
            sH.toString().indexOf("upx") >= 0
              ? parseInt(sH) * this.pxRatio
              : parseInt(sH);
          this.sS.width = sW + "px";
